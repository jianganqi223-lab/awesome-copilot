# Launch-App.ps1
# Generic application launcher — searches Start Menu, common paths, and direct path.
...
if ($PSCmdlet.ParameterSetName -eq 'Help') {
    Write-Host @"
Launch-App.ps1 — Launch any application on Windows

Usage:
  .\launch-app.ps1 -AppName "应用名称"     # Search and launch by name
...
Examples:
  ...
  .\launch-app.ps1 -AppName "计算器"
  .\launch-app.ps1 -AppName "notepad"
  .\launch-app.ps1 -AppPath "C:\Program Files\bilibili\哔哩哔哩.exe"
"@
    exit 0
}
...
if ($List) {
    ...
    Write-Host "  Chat:          wechat, 微信, qq"
    Write-Host "  Media:         bilibili, 哔哩哔哩, spotify"
...
$knownApps = @{
    ...
    'bilibili'    = @("C:\Program Files\bilibili\哔哩哔哩.exe")
    ...
    '微信'        = @("$env:ProgramFiles\Tencent\WeChat\WeChat.exe",
                       "${env:ProgramFiles(x86)}\Tencent\WeChat\WeChat.exe")
